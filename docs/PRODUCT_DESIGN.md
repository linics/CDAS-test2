...
- 通用 AI 预览：保持独立链路，按主学科/融合学科做加权检索，主学科优先。
- 教案一键生成：先做“基础字段抽取”（学段、年级、类型、深度、提交模式、主副学科），再按主副学科加权检索并注入提示词。
- 两条链路都返回可观测元数据：`meta.source/prompt_version/used_rag/fallback_reason`。
- 检索与 rerank 启用输入长度防护，避免超长导致外部 API 400。
...
- `assignment.preview@v2.3.0`
- `assignment.from_lesson_plan@v2.3.0`
...
- 成功/警告/错误统一右下可关闭气泡
- 超时错误给出明确下一步建议
- AI 生成过程增加步骤反馈气泡（当前阶段可见，减少等待焦虑）
- 反馈气泡尺寸与可读性升级（更大宽度与正文字号）
