...
export function Auth() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!IDENTIFIER_REGEX.test(trimmedIdentity)) {
      setError(`${identityLabel}格式应为 4-32 位字母、数字、下划线或连字符`);
      return;
    }
    ...
    if (role === "student" && (!Number.isFinite(studentGrade) || studentGrade < 1 || studentGrade > 9)) {
      setError("学生年级需在 1-9 之间");
      return;
    }
